...
## chenhy has modified this project and add a spinalHDL docker env

This env support using without internet connection.

There are two file: 
- this MySinalProj.zip project file
- spinalenvfull.tar docker env file

Make sure u can use docker.
Start the spinalHDL env in docker using: 

```
make build_env [ENV_NAME=...] [IMG_TAR_FILE=...]
```
You need to specify `spinalenvfull.tar` path in `IMG_TAR_FILE`. This will start a running container naming `$(ENV_NAME)`. This container will be running untill u remove the env. 
Remove the env using:
```
make rm_env [ENV_NAME=...]
```
While the env is running, u can use `make run` to build your project and verilog file will generated in `hw/gen`.
